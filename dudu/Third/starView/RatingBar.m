...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        //设置星星宽度是view宽度的
        CGFloat width = frame.size.width/(StarNumberMax+2);
        ...
        for(int i = 0;i< StarNumberMax;i++){
            ...
            img.image = [UIImage imageNamed:@"评价未选中"];
            //把该imageview加到底部视图上
            ...
            img2.image = [UIImage imageNamed:@"评价选中"];
            //把该imageview加到顶部视图上
...
-(void)tap:(UITapGestureRecognizer *)gesture{
    ...
    if(self.enable){
        ...
        if(count>StarNumberMax){
            _starNumber = StarNumberMax;
        }else{
            _starNumber = count-1;
        }
        [self starnumberBlock];
    }
}
//拖拽手势触发的方法
-(void)pan:(UIPanGestureRecognizer *)gesture{
    if(self.enable){
        ...
        if(count>=0 && count<=StarNumberMax && _starNumber!=count){
...
- (void)starnumberBlock{
    //结果回调
    if (self.starNumberBlock) {
        self.starNumberBlock(_starNumber);
    }
}
